...
import type { ChatSessionSummary } from "../lib/convex/api";
import {
...
export default function ChatHistorySidebar({ onSelectSession, currentSessionId }: Props) {
  ...
  const refreshSessions = useCallback(() => {
    ...
    setLoading(true);
    setError(null);
    listChatSessionsConvex('explore')
      .then(setSessions)
      .catch(err => {
        console.error(err);
        setError("We couldn't load your saved chats.");
      })
  ...
  const handleDeleteSession = async (sessionId: string) => {
    ...
    try {
      await deleteChatSessionConvex(sessionId);
  ...
  const handleClearAll = async () => {
    ...
    setClearing(true);
    try {
      await clearExploreSessionsConvex(currentSessionId ?? undefined);
      setSessions(prev => (currentSessionId != null
        ? prev.filter(s => s._id === currentSessionId)
        : []));
    } catch (err) {
      console.error(err);
              ...
              <li key={session._id}>
                <div
                  ...
                  className={`w-full px-4 py-3 hover:bg-slate-50 transition-colors group cursor-pointer ${
                    currentSessionId === session._id ? "bg-blue-50/50 border-l-2 border-blue-500" : "border-l-2 border-transparent"
                  }`}
                >
                  <div className="flex items-start gap-2">
                    <FiMessageSquare className={`h-3 w-3 mt-0.5 ${
                      currentSessionId === session._id ? "text-blue-500" : "text-slate-300 group-hover:text-slate-400"
                    }`} />
                    <div className="flex-1 min-w-0 flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        <div className={`text-xs font-medium truncate ${
                          currentSessionId === session._id ? "text-blue-700" : "text-slate-700"
                        }`}>
                          {session.title}
                        </div>
                      ...
                      <button
                        ...
                        disabled={deletingId === session._id || clearing}
